...
export class WebMidiAdapter {
  ...
  private readonly statusListeners = new Set<(status: WebMidiStatus) => void>();
  ...
  onStatusChange(cb: (status: WebMidiStatus) => void): () => void {
  ...
  private bindInputs(): void {
    ...
    this.status.connected = names.length > 0;
    if (!names.length) {
      this.inputManager.clearSource("web-midi");
    }
    this.emitStatus();
